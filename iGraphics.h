/*
	...
	two utility function added: iShowBMPRotate, iPassiveMouseMove
*/
...
int iAnimDelays[10];
int iAnimPause[10];
GLuint    textures[1];//(NEW) Storage for one texture.

void iDraw();
void iKeyboard(unsigned char);
void iSpecialKeyboard(unsigned char);
void iMouseMove(int, int);
void iPassiveMouseMove(int, int); //shahrear iqbal
...
void iShowBMPRotate(int x, int y, int degree, int size, LPSTR filename) //Shahrear Iqbal, to rotate an image
{
    ...
    glBindTexture(GL_TEXTURE_2D, textures[0]);// (NEW) used to select the texture you wanted to use

    glPushMatrix();
glTranslatef(x,y,0);
    glRotatef(degree, 0,0,1);//Rotate the square on the Z axis according to the rotation variable
    ...
    glVertex2f(-4*size,4*size);//This is the upper left corner of the square
    glTexCoord2f(1,1);//This is the upper right corner of the bitmap
    glVertex2f(4*size,4*size);
...
bool loadbmp(UINT textureArray[], LPSTR strFileName, int ID)//(NEW) the name of our function (Shahrear Iqbal)
{
...
void mousePassiveMoveHandlerFF(int mx, int my) //shahrear iqbal
{
	iMouseX = mx;
	iMouseY = iScreenHeight - my;
...
void iInitialize(int width=500, int height=500, char *title="iGraphics")
{
	...
	glutPassiveMotionFunc(mousePassiveMoveHandlerFF);	//shahrear iqbal
	glutIdleFunc(animFF) ;
